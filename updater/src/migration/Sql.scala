...
object Sql {
  ...
  def get_string(conn: Connection, sql: String) = {
  ...
  def get_int(conn: Connection, sql: String) = {
    val ps = conn.createStatement()
    // Printer.msg(s"$sql")
  ...
  def get_int2(conn: Connection, sql: String) = {
    val ps = conn.createStatement()
    // Printer.msg(s"$sql")
  ...
  def get_longset(conn: Connection, sql: String) = {
  ...
  def commit_enable() = commit_mode(true)
  def commit_disable() = commit_mode(false)
